...
class Board {
    ...
    public Point returnBestMove() {//최고값 
        int MAX = -100000;
        int best = -1;

        for (int i = 0; i < rootsChildrenScores.size(); ++i) { 
            if (MAX < rootsChildrenScores.get(i).score) {
                MAX = rootsChildrenScores.get(i).score;//애기들중에 젤 큰값을찾아
                best = i;//그럼 그 애기가 best
            }
        }
    ...
    public int minimax(int depth, int turn ,int alpha ,int beta) {
        ...
        List<Point> pointsAvailable = getAvailableStates(); // 사용가능한 포인트들
...
public class TicTacToe {
    ...
    public static void main(String[] args) {
        ...
        while (!b.isGameOver()) {
            ...
            System.out.println("머가 베스트인뎅"+b.returnBestMove());
            b.displayBoard();
        }
        if (b.hasXWon()) {
